...
ROLES = {
    'user': 0, # может только оставлять пожелания
...
def login_required(role=None):
    def wrapper(fn):
        ...
        def decorated_view(*args, **kwargs):
            if 'user_role' not in session or (role is not None and session['user_role'] < role):
    ...
    if request.method == 'POST':
        ...
        try:
          conn.execute("INSERT INTO users (username, password, role) VALUES (?, ?, ?)",
                (username, hashed_password, ROLES['user']))
          conn.commit()
          return redirect(url_for('login'))
...
def login():
    if request.method == 'POST':
        ...
        if user:
             ...
             return render_template('login.html', error='Неверные учетные данные. Проверьте логин и пароль или зарегистрируйтесь')
...
@login_required(role=ROLES['user'])
def add_wish():
    if request.method == 'POST':
